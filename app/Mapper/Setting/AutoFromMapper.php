...
class AutoFromMapper
{
    ...
    {
        $table = GenerateTables::find($table_id);
        if ($table->type != 'tree') {
            return [];
        }
    ...
    public function listQuerySetting(mixed $table_id, ?array $params, bool $isScope): Builder
    {
        // 使用Db
        $table = GenerateTables::find($table_id);
        $pkColumn = GenerateColumns::query()->where(['is_pk' => 2, 'table_id' => $table_id])->first();
        $columns = GenerateColumns::query()->where('table_id', '=', $table_id)->get();
    ...
    public function save(mixed $table_id, array $data): mixed
    {
        ...
        $columns = GenerateColumns::query()->pluck('column_name')->toArray();
        $removePk = false;
                ...
                foreach ($roles as $role) {
                    switch ($role->data_scope) {
                        ...
                        case Role::DEPT_BELOW_SCOPE:
                            ...
                            $this->userIds[] = $this->userid;
                            break;
                        case Role::DEPT_BELOW_SCOPE_BY_TABLE_DEPTID:
                            $parentDepts = Db::table('system_user_dept')->where('user_id', $userModel->id)->pluck('dept_id')->toArray();
                            $ids = [];
                            foreach ($parentDepts as $deptId) {
                                $ids[] = Dept::query()
