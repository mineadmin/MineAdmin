...
use App\Mapper\Setting\GenerateTablesMapper;
use App\Model\Setting\GenerateTables;
use App\Service\System\DataMaintainService;
...
class GenerateTablesService extends AbstractService
{
    ...
    public function loadTable(array $params): bool
    {
        ...
        foreach ($params['names'] as $item) {
            ...
            $this->generateColumnsService->save($columns);
        }
        return true;
    }
    ...
    public function sync(mixed $id): bool
    {
        ...
        $model = $this->generateColumnsService->mapper->getModel();
        $ids = $model->newQuery()->where('table_id', $table['id'])->pluck('id');

        $this->generateColumnsService->mapper->delete($ids->toArray());
        foreach ($columns as &$column) {
            $column['table_id'] = $id;
        }
        $this->generateColumnsService->save($columns);
    ...
    public function updateTableAndColumns(array $data): bool
    {
        ...
        // 更新业务字段表
        foreach ($columns as $column) {
            $this->generateColumnsService->update($column['id'], $column);
        }
        return true;
    }
