...
// import { getRolePermission, setRolePermission } from '~/base/api/department.ts'
...
useForm('departmentForm').then(async (form: MaFormExpose) => {
  const names: string[] = []
  if (data?.id) {
...
<template>
  <ma-form ref="departmentForm" v-model="userModel" />
</template>
